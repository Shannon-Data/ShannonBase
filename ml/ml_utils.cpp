...
std::map<std::string_view, ML_TASK_TYPE_T, std::less<>> OPT_TASKS_MAP = {
    ...
    {"LOG_ANOMALY_DETECTION", ML_TASK_TYPE_T::LOG_ANOMALY_DETECTION},
    {"RECOMMENDATION", ML_TASK_TYPE_T::RECOMMENDATION},
    {"TOPIC_MODELING", ML_TASK_TYPE_T::TOPIC_MODELING}};
...
std::map<ML_TASK_TYPE_T, std::string> TASK_NAMES_MAP = {
    ...
    {ML_TASK_TYPE_T::LOG_ANOMALY_DETECTION, "LOG_ANOMALY_DETECTION"},
    {ML_TASK_TYPE_T::RECOMMENDATION, "RECOMMENDATION"},
    {ML_TASK_TYPE_T::TOPIC_MODELING, "TOPIC_MODELING"}};
...
void Utils::parse_logad_options(Json_wrapper &options, std::string &additional_masking_regex, int &window_size,
  ...
  MYSQL_LEX_CSTRING key_logad = {STRING_WITH_LEN(ML_KEYWORDS::logad_options)};
  Json_wrapper logad_wrapper = options.lookup(key_logad);
  if (logad_wrapper.empty() || logad_wrapper.type() != enum_json_type::J_OBJECT) return;

  OPTION_VALUE_T opt_values;
  std::string keystr;
  parse_json(logad_wrapper, opt_values, keystr, 0);

  auto it = opt_values.find(ML_KEYWORDS::additional_masking_regex);
  ...
  it = opt_values.find(ML_KEYWORDS::embedding_model);
  if (it != opt_values.end() && !it->second.empty()) embedding_model = it->second[0];

  it = opt_values.find(ML_KEYWORDS::keyword_model);
  if (it != opt_values.end() && !it->second.empty()) keyword_model = it->second[0];

  if (embedding_model.empty() && keyword_model.empty()) {
    my_error(ER_ML_FAIL, MYF(0), "embedding_model and keyword_model cannot both be NULL for log_anomaly_detection");
  }
}

int Utils::validate_table_size(TABLE *table) {
  if (!table) return HA_ERR_GENERIC;

  // Check column count (max 1017)
  if (table->s->fields > 1017) {
    std::ostringstream err;
    err << "Table has " << table->s->fields << " columns, exceeding the maximum of 1017";
  ...
  if (estimated_rows > 100000000) {
    std::ostringstream err;
    err << "Table has approximately " << estimated_rows << " rows, exceeding the maximum of 100 million";
    my_error(ER_ML_FAIL, MYF(0), err.str().c_str());
    return HA_ERR_GENERIC;
  }
  ...
  if (table_size_bytes > MAX_TABLE_SIZE) {
    std::ostringstream err;
    err << "Table size (" << (table_size_bytes / (1024 * 1024 * 1024)) << " GB) exceeds the maximum of 10 GB";
    my_error(ER_ML_FAIL, MYF(0), err.str().c_str());
    return HA_ERR_GENERIC;
  }
  return 0;
...
int Utils::validate_target_not_text(TABLE *table, const std::string &target_name) {
  if (target_name.empty()) return 0;
  for (uint i = 0; i < table->s->fields; i++) {
    Field *field = table->field[i];
    if (field->field_name == target_name) {
      // TEXT types: BLOB, MEDIUM_BLOB, LONG_BLOB, TINY_BLOB
      if (field->type() == MYSQL_TYPE_BLOB || field->type() == MYSQL_TYPE_MEDIUM_BLOB ||
        ...
        std::ostringstream err;
        err << "MySQL HeatWave AutoML does not support a text target column: " << target_name;
        my_error(ER_ML_FAIL, MYF(0), err.str().c_str());
        return HA_ERR_GENERIC;
      }
      return 0;
...
void Utils::apply_exclude_model_list(std::vector<std::string> &model_list,
                                     const std::vector<std::string> &exclude_model_list) {
  if (exclude_model_list.empty()) return;
  model_list.erase(std::remove_if(model_list.begin(), model_list.end(),
                                  [&](const std::string &m) {
                                    return std::find(exclude_model_list.begin(), exclude_model_list.end(), m) !=
                                           exclude_model_list.end();
                                  }),
                   model_list.end());
}
...
TABLE *Utils::open_table_by_name(const std::string &schema_name, const std::string &table_name, thr_lock_type lk_mode) {
  ...
  // if (table_list.table && table_list.table->s) {
  //    table_list.table->alias = table_list.table->s->table_name.str;
  // }
...
// a new param, check_condition needed? std::function<void(double)> ? to check the data must satisfy the the condition.
int Utils::read_data(TABLE *table, std::vector<double> &train_data, std::vector<std::string> &features_name,
  ...
  const dd::Table *table_def{nullptr};
  {
    ...
    if (thd->dd_client()->acquire(table->s->db.str, table->s->table_name.str, &table_def) || !table_def) {
      my_error(ER_ML_FAIL, MYF(0), "ML read_data: failed to acquire dd::Table");
      return 0;
    }
  }

  unique_ptr_destroy_only<handler> sec_handler(Utils::get_secondary_handler(table));
  if (!sec_handler) {
    my_error(ER_ML_FAIL, MYF(0), "ML read_data: failed to get secondary handler");
    return 0;
  }
  if (sec_handler->ha_open(table, table->s->normalized_path.str, O_RDONLY, HA_OPEN_IGNORE_IF_LOCKED, table_def)) {
    my_error(ER_ML_FAIL, MYF(0), "ML read_data: ha_open failed on secondary engine");
    return 0;
  }
  if (sec_handler->ha_external_lock(thd, F_RDLCK)) {
    sec_handler->ha_close();
    return 0;
  }

  ShannonBase::Utils::ColumnMapGuard col_guard(table, ShannonBase::Utils::ColumnMapGuard::TYPE::READ);
  if (sec_handler->ha_rnd_init(true)) {
    sec_handler->ha_external_lock(thd, F_UNLCK);
    sec_handler->ha_close();
    return 0;
  }
  ...
  int rc;
  while ((rc = sec_handler->ha_rnd_next(table->record[0])) != HA_ERR_END_OF_FILE) {
    if (rc != 0) break;
...
Json_object *Utils::build_up_model_metadata(
  ...
  model_obj->add_alias(ML_KEYWORDS::training_params, new (std::nothrow) Json_string(training_params));
  model_obj->add_alias(ML_KEYWORDS::onnx_inputs_info, onnx_inputs_info);
  model_obj->add_alias(ML_KEYWORDS::onnx_outputs_info, onnx_outputs_info);
  model_obj->add_alias(ML_KEYWORDS::training_drift_metric, training_drift_metric);

  model_obj->add_alias(ML_KEYWORDS::task, new (std::nothrow) Json_string(task));
  model_obj->add_alias(ML_KEYWORDS::build_timestamp, new (std::nothrow) Json_double(now_seconds));
  model_obj->add_alias(ML_KEYWORDS::target_column_name, new (std::nothrow) Json_string(target_column_name));
  model_obj->add_alias(ML_KEYWORDS::train_table_name, new (std::nothrow) Json_string(tain_table_name));

  Json_array *column_names_arr = new (std::nothrow) Json_array();
  for (auto &feature_name : featurs_name) column_names_arr->append_alias(new (std::nothrow) Json_string(feature_name));
  model_obj->add_alias(ML_KEYWORDS::column_names, column_names_arr);

  model_obj->add_alias(ML_KEYWORDS::model_explanation, model_explanation);

  model_obj->add_alias(ML_KEYWORDS::notes, new (std::nothrow) Json_string(notes));
  model_obj->add_alias(ML_KEYWORDS::format, new (std::nothrow) Json_string(format));
  model_obj->add_alias(ML_KEYWORDS::status, new (std::nothrow) Json_string(status));

  model_obj->add_alias(ML_KEYWORDS::model_quality, new (std::nothrow) Json_string(model_quality));
  ...
  for (auto &sel_col_name : selected_column_names)
    selected_column_names_arr->append_alias(new (std::nothrow) Json_string(sel_col_name));
  model_obj->add_alias(ML_KEYWORDS::selected_column_names, selected_column_names_arr);

  model_obj->add_alias(ML_KEYWORDS::contamination, new (std::nothrow) Json_double(contamination));
  model_obj->add_alias(ML_KEYWORDS::chunks, new (std::nothrow) Json_double(chunks));

  if (!txt2num_dict.empty()) {
    ...
    for (const auto &iter : txt2num_dict) {
      ...
      for (const auto &val : iter.second) value_arr->append_alias(new (std::nothrow) Json_string(val));
      txt2num_dict_obj->add_alias(iter.first.c_str(), value_arr);
    }
    model_obj->add_alias(ML_KEYWORDS::txt2num_dict, txt2num_dict_obj);
  }
  return model_obj;
}
...
int Utils::ML_train(std::string &task_mode, uint data_type, const void *training_data, uint n_data,
   ...
   */
  model_content_buff = std::make_unique<char[]>(bufflen + 1);
  memset(model_content_buff.get(), 0x0, bufflen + 1);
  ...
  {  // start to assemble the model object json file. If the model content is not JSON format, therefore, we assemble it
    ...
    model_obj->add_alias(ML_KEYWORDS::SHANNON_LIGHTGBM_CONTENT, new (std::nothrow) Json_string(model_content));
...
Json_object *Utils::compute_permutation_importance(std::string &model_content_json,
  ...
  // Permutation importance is only meaningful for classification tasks.
  // For regression and forecasting, the binary accuracy metric is nonsensical.
  if (task_type != ML_TASK_TYPE_T::CLASSIFICATION) return nullptr;
  ...
  {
    Json_dom_ptr dom = Json_dom::parse(
        model_content_json.c_str(), model_content_json.length(), [](const char *, size_t) { assert(false); },
        [] { assert(false); });
  ...
  // Determine number of classes (1 for binary/regression, >1 for multi-class)
  int num_classes = 1;
  int ret_nc = LGBM_BoosterGetNumClasses(booster, &num_classes);
  if (ret_nc == -1) num_classes = 1;  // fallback: treat as binary
  bool is_multiclass = (num_classes > 1);
  size_t preds_per_sample = is_multiclass ? static_cast<size_t>(num_classes) : 1;

  // Helper: predict and compute accuracy-based score
  auto predict_and_score = [&](const std::vector<double> &data) -> double {
    std::vector<double> preds(n_samples * preds_per_sample, 0.0);
    int64_t out_len;
    std::string params;
    int ret = LGBM_BoosterPredictForMat(booster, data.data(), C_API_DTYPE_FLOAT64, n_samples, n_features, 1,
                                        C_API_PREDICT_NORMAL, 0, -1, params.c_str(), &out_len, preds.data());
    if (ret) return -1.0;
    ...
    for (size_t i = 0; i < n_samples; i++) {
      int pred_class = 0;
      if (is_multiclass) {
        // Argmax over num_classes raw scores per sample
        double max_val = preds[i * num_classes];
        for (int c = 1; c < num_classes; c++) {
          if (preds[i * num_classes + c] > max_val) {
            max_val = preds[i * num_classes + c];
            pred_class = c;
          }
        }
      } else {
        // Binary classification threshold
        pred_class = (preds[i] >= 0.5) ? 1 : 0;
      }
  ...
  if (wrapper)
    wrapper->add_alias("permutation_importance", result);
  else
