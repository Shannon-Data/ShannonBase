...
int table_rpd_column_id::make_row(uint index[[maybe_unused]]) {
  ...
  if (index >= meta_rpd_columns_infos.size()) {
    ...
    m_row.column_id = meta_rpd_columns_infos[index].column_id;
    m_row.table_id = meta_rpd_columns_infos[index].table_id;
    strncpy(m_row.column_name, meta_rpd_columns_infos[index].table_name, 
            strlen(meta_rpd_columns_infos[index].table_name));
    m_row.column_name_length = strlen(meta_rpd_columns_infos[index].table_name);
  }
  return 0;
}
