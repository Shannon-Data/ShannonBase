...
bool Optimizer::translate_access_path(TranslateState *state, THD *thd, AccessPath *path, const JOIN *join) {
  ...
  switch (path->type) {
    ...
    case AccessPath::INDEX_RANGE_SCAN: {
      ...
      /*
        A partitioned table is read by MySQL's own TableScanIterator, not by
        VectorizedTableScanIterator (ScanTable::ToAccessPath() clears the flag
        again from the data dictionary). Say so here, before any rule runs:
        the native iterator cannot apply a pushed predicate, so predicate
        pushdown has to know not to move a WHERE into this scan and leave
        nothing to re-apply it.
      */
      if (share->is_partitioned) scan->vectorized = false;
