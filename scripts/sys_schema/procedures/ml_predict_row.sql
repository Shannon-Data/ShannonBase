...
in_data JSON:
  Specifies the data to generate explanations for. Data must be specified in JSON key-value format, where the key is a column name. The column names must match the feature column names in the table used to train the model.
in_model_handle_name (VARCHAR(256)):
...
Example
-----------
mysql> CALL sys.ML_PREDICT_ROW(JSON_OBJECT(\'in_sch_tb_name\', \'in_model_handle_name\',
          in_model_option);
    '
...
BEGIN
    ...
    SELECT ML_MODEL_PREDICT_ROW(in_data, in_model_handle_name, in_model_option) INTO v_pred_table_obj_check;
