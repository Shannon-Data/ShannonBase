...
DROP FUNCTION IF EXISTS shannon_chat;
...
function shannon_agent_run(user_message, conversation_id) {
  ...
  function ml_generate(prompt, extra) {
    ...
    // 优先使用 chat_opt.model_options，然后是 extra 参数，最后是默认值
    var model_opts = (chat_opt && chat_opt.model_options) ? chat_opt.model_options : {};
    
    var o = Object.assign({
      task: 'generation',
      model_id: 'Qwen3.5-2B-ONNX',
      language: 'zh',
      temperature: 0.25,
      max_tokens: 1200,
      top_p: 0.95,
  ...
  function call_schema_metadata(user_msg, db, n_results, extra_opts) {
    ...
    try {
      ...
      /* procedure 本身报错（如 embedding 服务不可用）→ 静默返回空，触发降级 */
      return '';
    }
  }
  ...
  function build_schema_context_fallback(db, budget) {
    ...
    try {
      ...
      var lines = ['【Schema（fallback，schema_embeddings 未就绪）: ' + db + '】'];
      var total = 0;
      if (tbl_rows.length > MAX_TABLES) {
        ...
        for (var ti = 0; ti < tbl_rows.length; ti++) {
          ...
          var descs = [];
          var show  = Math.min(cols.length, MAX_COLS);
          for (var ci2 = 0; ci2 < show; ci2++) {
            ...
            if (col.COLUMN_COMMENT)
              desc += '/*' + col.COLUMN_COMMENT.substring(0,30) + '*/';
            descs.push(desc);
          }
  ...
  function build_schema_context(db, available_tokens, chat_opt) {
    ...
    /* schemas_override：允许 @chat_options 指定额外 schema 过滤 */
    var extra = {};
    ...
    if (check_schema_embeddings_ready(db)) {
      ...
      if (result && result.length > 0) {
        return '【相关表 DDL（按语义相关性排序，FOREIGN KEY 可直接用于 JOIN）】\n' + result;
      }
    }
  ...
  function suggest_joins(graph, tables) {
    ...
    return hints.length > 0 ? '【推荐JOIN路径（BFS，仅在 schema_embeddings 未就绪时使用）】\n' +
                               hints.join('\n') : '';
  }

  function save_chat_options(chat_opt) {
    _cached_chat_opt = chat_opt;  // 更新缓存
  ...
  function discover_vector_tables(chat_opt) {
    ...
    if (chat_opt.vector_store_columns) {
      try {
        var vsc = typeof chat_opt.vector_store_columns === 'string'
                  ? JSON.parse(chat_opt.vector_store_columns) : chat_opt.vector_store_columns;
        seg_col = vsc.segment || seg_col; emb_col = vsc.segment_embedding || emb_col;
      } catch(e) {}
    }
    ...
    return rows.map(function(r) { return { schema_name: r.TABLE_SCHEMA, table_name: r.TABLE_NAME }; });
  }
  ...
  function save_memory(conv_id, role, content, thought, with_embedding) {
      ...
      if (with_embedding) {
          sys.exec_sql(
              ...
              "SELECT '" + esc(conv_id) + "','" + esc(role) + "','" + esc(content) + "','" + esc(thought || '') + "', " +
              "sys.ML_EMBED_ROW('" + esc(content) + "', JSON_OBJECT('model_id','all-MiniLM-L12-v2','truncate',true))"
          );
      } else {
          sys.exec_sql(
              "INSERT INTO mysql.agent_memory(conversation_id, role, content, thought, embedding) " +
              "VALUES('" + esc(conv_id) + "','" + esc(role) + "','" + esc(content) + "','" + esc(thought || '') + "', NULL)"
          );
      }
  }

  function save_vector_memory(role, content, thought) {
      save_memory(conversation_id, role, content, thought, true);
  }
  ...
  function replace_ph(sql, db) {
    if (!db) return sql;
    return sql.replace(/your_database_name|your_db_name|<database_name>|\[database_name\]|\{database_name\}|your_schema/gi, db);
  }
  ...
  function parse_explain(json_str) {
    try {
      ...
      function walk(node) {
        ...
        if (node.access_type) { out.push('访问=' + node.access_type); if (node.access_type==='ALL') out.push('⚠全表扫描'); }
        ...
        ['nested_loop','attached_subqueries','query_block','ordering_operation','grouping_operation'].forEach(function(k) {
          if (!node[k]) return;
  ...
  function execute_tool(tool, args, db) {
    ...
    if (tool === 'query_db') {
      ...
      if (!RO[first]) return '拒绝：query_db 只允许只读语句。';
  ...
  function build_system_prompt(db, schema_ctx, join_hint, plan_hint,
    ...
    return (
      '你是 ShannonBase Agent v3.2，内置于 ShannonBase 数据库的智能 SQL 助手。\n' +
      ...
      '3. {"thought":"...","tool":"begin_tx","args":{}}\n' +
      '4. {"thought":"...","tool":"update_data","args":{"sql":"INSERT INTO ..."}}\n' +
      '5. {"thought":"...","tool":"commit_tx","args":{}}\n' +
      '6. {"thought":"...","tool":"rollback_tx","args":{}}\n' +
      '7. {"thought":"...","tool":"ml_rag","args":{"question":"...","top_k":6}}\n' +
      '8. {"thought":"...","tool":"generate_text","args":{"prompt":"..."}}\n\n' +
  ...
  for (var turn = 0; turn < MAX_TURNS; turn++) {
    var llm_out  = ml_generate(full_prompt, { max_tokens: 400 });
    var tool_obj = parse_tool_call(llm_out);
    if (!tool_obj) { agent_response = llm_out.trim() || last_result; need_summary = false; break; }

    var result  = execute_tool(tool_obj.tool, tool_obj.args || {}, current_db);
    last_result = result; need_summary = true;
