...
MySQLServerRapid::MySQLServerRapid(net::io_context &io_ctx,
...
void MySQLServerRapid::close_all_connections() {
...
class Acceptor {
 public:
  ...
  Acceptor(net::io_context &io_ctx, std::string protocol_name,
           WaitableMonitor<std::list<std::unique_ptr<MySQLServerRapidSession>>>
               &client_sessions,
  ...
  void accepted(protocol_type::socket client_sock) {
    ...
    auto session_it = client_sessions_([&](auto &socks) {
      if (protocol_name_ == "classic") {
        socks.emplace_back(std::make_unique<MySQLServerRapidSessionClassic>(
            MySQLClassicProtocol{std::move(client_sock), client_ep_,
                                 tls_server_ctx_},
            std::move(reader), false, with_tls_));
      } else {
        socks.emplace_back(std::make_unique<MySQLServerRapidSessionX>(
...
void MySQLServerRapid::run(mysql_harness::PluginFuncEnv *env) {
  Acceptor acceptor{
      io_ctx_,
      protocol_name_,
      client_sessions_,
      DuktapeStatementReaderFactory{
          ...
          RapidServerComponent::get_instance().get_global_scope()},
      tls_server_ctx_,
