...
#include "router/src/shannon_rapid_server/src/statement_reader.h"
...
MySQLServerRapidSessionX::clock_type::time_point
MySQLServerRapidSessionX::notice_expiry() const {
...
bool MySQLServerRapidSessionX::encode_due_async_notices(
    const MySQLServerRapidSessionX::clock_type::time_point &start_time) {
...
MySQLServerRapidSessionX::MySQLServerRapidSessionX(
    MySQLXProtocol protocol,
    std::unique_ptr<StatementReaderBase> statement_processor,
    const bool debug_mode, bool with_tls)
    : MySQLServerRapidSession(std::move(statement_processor), debug_mode),
      async_notices_(this->json_reader_->get_async_notices()),
      protocol_{std::move(protocol)},
      with_tls_{with_tls} {}

void MySQLServerRapidSessionX::send_response_then_handshake() {
...
void MySQLServerRapidSessionX::greeting() {
...
void MySQLServerRapidSessionX::handshake() {
  std::vector<uint8_t> payload;
...
void MySQLServerRapidSessionX::send_response_then_disconnect() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerRapidSessionX::send_response_then_first_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerRapidSessionX::send_response_then_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerRapidSessionX::send_notice_then_notices() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerRapidSessionX::notices() {
...
void MySQLServerRapidSessionX::idle() {
...
void MySQLServerRapidSessionX::run() { greeting(); }
