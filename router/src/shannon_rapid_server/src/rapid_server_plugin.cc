...
static constexpr const char kSectionName[]{"rapid_server"};
...
static void start(mysql_harness::PluginFuncEnv *env) {
  ...
  my_thread_self_setname(std::string("RapidSrv:" + section->key).c_str());
  ...
  try {
    ...
    auto srv = std::make_shared<server_rapid::MySQLServerRapid>(
    ...
    RapidServerComponent::get_instance().register_server(key, srv);
...
extern "C" {
mysql_harness::Plugin RAPID_SERVER_EXPORT harness_plugin_rapid_server = {
    mysql_harness::PLUGIN_ABI_VERSION,       // abi-version
    mysql_harness::ARCHITECTURE_DESCRIPTOR,  // arch
    "Rapid MySQL Server for testing",         // name
