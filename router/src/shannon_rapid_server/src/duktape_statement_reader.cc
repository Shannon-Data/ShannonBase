...
class DukHeap {
 public:
  DukHeap(std::vector<std::string> module_prefixes,
          std::shared_ptr<RapidServerGlobalScope> shared_globals)
...
// On Solaris we disable cache functionality as it causes rapid server crash:
// uncaught: 'invalid bytecode'
//     Application got fatal signal: 6
//              server_rapid::DukHeap::DukHeap(std::string const&,
//              std::shared_ptr<RapidServerGlobalScope>)::{lambda(void*, char
...
//              const*)#1})+0x28 [0xffffff02c0133bc8]
  ...
  std::shared_ptr<RapidServerGlobalScope> shared_;
...
static duk_int_t process_get_keys(duk_context *ctx) {
  ...
  auto *shared_globals =
      static_cast<RapidServerGlobalScope *>(duk_get_pointer(ctx, -1));
...
static classic_protocol::message::server::Greeting default_server_greeting() {
  // defaults
  std::string server_version = "8.0.23-rapid";
