...
static constexpr const char kSectionName[]{"rest_rapid_server"};
static constexpr const char kRestGlobalsUri[]{"^/api/v1/rapid_server/globals/$"};
...
class RestApiV1MockServerGlobals : public BaseRequestHandler {
 ...
 private:
  ...
  void handle_global_put_all(HttpRequest &req) {
    ...
    typename RapidServerGlobalScope::type all_globals;
...
class RestApiV1MockServerConnections : public BaseRequestHandler {
 ...
 private:
  ...
  void handle_connections_delete_all(HttpRequest &req) {
    // tell the rapid_server to close all connections
    RapidServerComponent::get_instance().close_all_connections();
...
#if defined(_MSC_VER) && defined(rest_rapid_server_EXPORTS)
...
extern "C" {
mysql_harness::Plugin DLLEXPORT harness_plugin_rest_rapid_server = {
