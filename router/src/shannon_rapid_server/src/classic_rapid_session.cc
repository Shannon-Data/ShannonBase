...
#include "classic_rapid_session.h"
...
void MySQLServerRapidSessionClassic::server_greeting() {
...
void MySQLServerRapidSessionClassic::client_greeting() {
...
void MySQLServerRapidSessionClassic::auth_switched() {
  // check we have enough of a client-greeting.
  std::vector<uint8_t> payload;
  auto frame_decode_res = protocol_.read_packet(payload);
  if (!frame_decode_res) {
...
void MySQLServerRapidSessionClassic::send_response_then_disconnect() {
...
void MySQLServerRapidSessionClassic::send_response_then_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerRapidSessionClassic::idle() {
  std::vector<uint8_t> payload;
  auto frame_decode_res = protocol_.read_packet(payload);
  if (!frame_decode_res) {
    auto ec = frame_decode_res.error();
...
void MySQLServerRapidSessionClassic::finish() { disconnect(); }

void MySQLServerRapidSessionClassic::run() { server_greeting(); }
...
stdx::expected<void, ErrorResponse> MySQLServerRapidSessionClassic::authenticate(
