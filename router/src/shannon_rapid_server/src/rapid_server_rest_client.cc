...
#include "mysqlrouter/rapid_server_rest_client.h"
...
RapidServerRestClient::RapidServerRestClient(const uint16_t http_port,
                                           const std::string &http_hostname)
    : http_hostname_(http_hostname), http_port_(http_port) {}

void RapidServerRestClient::set_globals(const std::string &globals_json) {
  IOContext io_ctx;
  RestClient rest_client(io_ctx, http_hostname_, http_port_);
  auto put_req = rest_client.request_sync(
      HttpMethod::Put, kRapidServerGlobalsRestUri, globals_json);
...
std::string RapidServerRestClient::get_globals_as_json_string() {
  IOContext io_ctx;
  auto req = RestClient(io_ctx, "127.0.0.1", http_port_)
                 .request_sync(HttpMethod::Get, kRapidServerGlobalsRestUri);
  if (!req) {
    throw std::runtime_error(std::string("GET ") + kRapidServerGlobalsRestUri +
                             " @ " + http_hostname_ + ":" +
                             std::to_string(http_port_) +
...
int RapidServerRestClient::get_int_global(const std::string &global_name) {
...
bool RapidServerRestClient::get_bool_global(const std::string &global_name) {
  const auto json_payload = get_globals_as_json_string();
...
void RapidServerRestClient::send_delete(const std::string &uri) {
  IOContext io_ctx;
  RestClient rest_client(io_ctx, http_hostname_, http_port_);
  auto kill_req = rest_client.request_sync(HttpMethod::Delete,
                                           "/api/v1/rapid_server/connections/");
...
bool RapidServerRestClient::wait_for_rest_endpoint_ready(
  ...
  while (max_wait_time.count() > 0) {
    auto req =
        rest_client.request_sync(HttpMethod::Get, kRapidServerGlobalsRestUri);

    if (req && req.get_response_code() != 0 && req.get_response_code() != 404)
      return true;

    auto wait_time =
        std::min(kRapidServerMaxRestEndpointStepTime, max_wait_time);
