...
class MysqlServerMockFrontend {
 public:
  ...
  std::string get_help() {
    ...
    for (auto line : arg_handler_.usage_lines("Usage: mysql_server_rapid", "",
                                              kHelpScreenWidth)) {
      os << line << std::endl;
    }
  ...
  void run() {
    ...
    mysql_harness::logging::create_module_loggers(
        registry, log_level,
        {mysql_harness::logging::kMainLogger, "rapid_server", "http_server", "",
         "rest_rapid_server"},
        mysql_harness::logging::kMainLogger);
    mysql_harness::logging::create_main_log_handler(
        registry, "rapid_server", config_.logging_folder, true);
    ...
    const std::string logfile_name = "rapid_server_" + config_.port + ".log";
    ...
    if (!config_.http_port.empty()) {
      auto &rest_rapid_server_config =
          loader_config->add("rest_rapid_server", "");
      rest_rapid_server_config.set("library", "rest_rapid_server");
    ...
    auto &rapid_server_config = loader_config->add("rapid_server", "classic");
    rapid_server_config.set("library", "rapid_server");
    rapid_server_config.set("bind_address", config_.bind_address);
    rapid_server_config.set("port", config_.port);
    rapid_server_config.set("filename", config_.queries_filename);
    rapid_server_config.set("module_prefix", config_.module_prefix);
    rapid_server_config.set("protocol", "classic");
    rapid_server_config.set("ssl_mode", config_.ssl_mode);
    rapid_server_config.set("ssl_cert", config_.ssl_cert);
    rapid_server_config.set("ssl_key", config_.ssl_key);
    rapid_server_config.set("tls_version", config_.tls_version);
    rapid_server_config.set("ssl_cipher", config_.ssl_cipher);
    rapid_server_config.set("ssl_ca", config_.ssl_ca);
    rapid_server_config.set("ssl_capath", config_.ssl_capath);
    rapid_server_config.set("ssl_crl", config_.ssl_crl);
    rapid_server_config.set("ssl_crlpath", config_.ssl_crlpath);

    if (!config_.xport.empty()) {
      auto &rapid_x_server_config = loader_config->add("rapid_server", "x");
      rapid_x_server_config.set("library", "rapid_server");
      rapid_x_server_config.set("port", config_.xport);
      rapid_x_server_config.set("filename", config_.queries_filename);
      rapid_x_server_config.set("module_prefix", config_.module_prefix);
      rapid_x_server_config.set("protocol", "x");
      rapid_x_server_config.set("ssl_mode", config_.ssl_mode);
      rapid_x_server_config.set("ssl_cert", config_.ssl_cert);
      rapid_x_server_config.set("ssl_key", config_.ssl_key);
      rapid_x_server_config.set("tls_version", config_.tls_version);
      rapid_x_server_config.set("ssl_cipher", config_.ssl_cipher);
      rapid_x_server_config.set("ssl_ca", config_.ssl_ca);
      rapid_x_server_config.set("ssl_capath", config_.ssl_capath);
      rapid_x_server_config.set("ssl_crl", config_.ssl_crl);
      rapid_x_server_config.set("ssl_crlpath", config_.ssl_crlpath);
    }
    ...
    try {
      loader_ = std::make_unique<mysql_harness::Loader>("server-rapid",
                                                        *loader_config);
